...
pdfsFolder   = os.path.dirname(os.getcwd())
proofsFolder = os.path.dirname(pdfsFolder)
...
for glyphName in glyphNames:
    ...
    with savedState():
        ...
        fontSize(11)
        fill(1, 0, 0)
        text(glyphName, (width()/2, 10), align='center')
        rotate(90)
        W = height()/3
        for pt in [8, 14, 144]:
            text(f'{pt}', (W/2, -20), align='center')
            translate(W, 0)
    ...
    for i, wght in enumerate(_wght):
        for j, opsz in enumerate(_opsz):
            ...
            T.lineHeight(fs*1.2)
            T.align('center')
            for fontStyle in [fontRoman, fontItalic]:
                T.font(fontStyle)
                for wdth in _wdth:
                    variations = {
                        'opsz' : opsz,
